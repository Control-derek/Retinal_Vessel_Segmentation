...
class Tester(Trainer):
    def __init__(self, model, loss, CFG, checkpoint, test_loader, dataset_path, show=False):
        ...
        if self.show:
            dir_exists(f"save_picture/{self.start_time}")
            remove_files(f"save_picture/{self.start_time}")
            dir_exists(f"save_picture/{self.start_time}/img")
            remove_files(f"save_picture/{self.start_time}/img")
            dir_exists(f"save_picture/{self.start_time}/pre")
            remove_files(f"save_picture/{self.start_time}/pre")
            dir_exists(f"save_picture/{self.start_time}/pre_b")
            remove_files(f"save_picture/{self.start_time}/pre_b")
        ...
        with torch.no_grad():
            for i, (img, gt) in enumerate(tbar):
                ...
                if self.show:
                    ...
                    print(f'self.CFG.threshold{self.CFG.threshold}')
                    predict_b = np.where(predict >= self.CFG.threshold, 1, 0)
                    cv2.imwrite(
                        f"save_picture/{self.start_time}/img/{i+1}.png", np.uint8(img.cpu().numpy()*255))
                    # cv2.imwrite(
                    #     f"save_picture/{self.start_time}/gt/{i+1}.png", np.uint8(gt.cpu().numpy()*255))
                    cv2.imwrite(
                        f"save_picture/{self.start_time}/pre/{i+1}.png", np.uint8(predict*255))
                    cv2.imwrite(
                        f"save_picture/{self.start_time}/pre_b/{i+1}.png", np.uint8(predict_b*255))
