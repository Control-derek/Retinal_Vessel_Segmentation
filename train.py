...
def main(CFG, data_path, batch_size, with_val=False, finetune=False, weight_path=None):
    seed_torch()
...
if __name__ == '__main__':
    ...
    parser.add_argument("-ft", '--finetune', help="finetune",
                        required=False, default=False, action="store_true")
    parser.add_argument("-wp", '--weight_path', default=None, type=str,
                        required=False, help='the path of checkpoint')
    args = parser.parse_args()

    with open('config.yaml', encoding='utf-8') as file:
        CFG = Bunch(YAML(typ='safe', pure=True).load(file))
    main(CFG, args.dataset_path, args.batch_size, args.val, args.finetune, args.weight_path)
